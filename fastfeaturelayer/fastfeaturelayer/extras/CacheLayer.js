                    ...
                    for (var g_x = start_x; g_x <= end_x; g_x++) {
                        ...
                        redis.then(function (data) {
                            arrayUtils.forEach(data.features, function (fea) {
                                var sms = new SimpleMarkerSymbol("circle", 6, null, new Color("255,255,255,1"));
                                var g = new Graphic(new Point(fea.geometry.x, fea.geometry.y, map.spatialReference), sms, fea.attributes, null);
                                g.isGrid = false;
